...
T = readtable( strcat( path_data,'IIASA_all.csv' ) );
names1 = T( :, 1 : 2 );
...
alt_data = csvread( strcat( path_data, 'IIASA_all.csv' ), 0, 5 );
% convert to ppm in C
...
bau_data = csvread(strcat(path_data,'IIASA_BAU.csv'),0,5);
...
for scn = 1:Nalt
        ...
        print(strcat(path_pics,'emissionsPairs/IISASc_',num2str(scn),'_emissionPairs.png'), '-dpng')
        hold off
end
...
for method = methodVec
    I = [true ( strcmp(category, names_category(4)) | strcmp(category, names_category(5))...
                    | strcmp(category, names_category(6)) ) ];
    plot_Emissions( data_alt( :, I ), start_year_alt( I(2:end) ), PastTotalCO2emission, method,...
...
for method = methodVec
    I = [true ( strcmp(category, names_category(1)) | strcmp(category, names_category(2))...
                | strcmp(category, names_category(3)) ) ];
    plot_Emissions( data_alt( :, I ), start_year_alt( I(2:end) ), PastTotalCO2emission, method,...
                    [ -5, 10 ], 'IISA 1.5$^\circ$: CO2 emissions',...
