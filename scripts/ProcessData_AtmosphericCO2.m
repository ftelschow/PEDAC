...
h = ylabel("dtdelpCO2a\_obs");  set(h, 'Interpreter', 'latex');
h = legend('dtdelpCO2a\_obs','location','northwest');
...
grid
h = xlabel("years");  set(h, 'Interpreter', 'latex');
h = ylabel("dpCO2a\_obs");  set(h, 'Interpreter', 'latex');
h = legend('dpCO2a\_obs','location','northwest');
set(h, 'Interpreter', 'latex');
set(gca, 'fontsize', 14);
...
clear dtdelpCO2a_obs dpCO2a_obs h fig
...
minLoss = @(x) LSE_Params( x, PastTotalCemission_int, CO2a_obs, 1765, 2005 );
...
% xopt2 = [285.1693    0.8047];  % loss minimized 1958-2016
...
CO2a = CO2a2;
save( strcat(path_data, 'RafelskiModelFit_', num2str(year_s),'_',...
      num2str(year_e),'.mat'), 'CO2a', 'xopt')
...
for scenarioNum = 1:N2deg
    ...
    data_AR5              = [ AR52deg_data(:,1) AR52deg_data(:, scenarioNum+1) ] ;
    data_AR5              = data_AR5(Index_NoNmissing, :);
    % convert to ppm in C
    data_AR5(:,2)         = data_AR5(:,2) / 10^3 * gtonC_2_ppmC;
    cut_year(scenarioNum) = min(data_AR5(:,1));
    Ie                    = find(PastTotalCO2emission_int(:,1)==cut_year(scenarioNum));
    ...
    data_AR52deg( 1:(size(data_int,1)+size(data_obs,1)), scenarioNum+1 ) = ...
                        [data_obs(:,2); data_int(:,2)];
end

cut_year_2deg = cut_year;

% Clear workspace
clear Ia data_AR5 data_int data Index_NoNmissing AR52deg_data ...
...
cut_year          = zeros([1 Nbase]);

clear years monthTimes

% Interpolate the data to feed into the Joos Model
for scenarioNum = 2:(Nbase+1)
    % Pick scenario and find the missing data
    ...
    % convert to ppm in C
     data_AR5(:,2)     = data_AR5(:,2) / 10^3 * gtonC_2_ppmC;
    cut_year(scenarioNum-1) = min(data_AR5(:,1));
    Ie                = find(PastTotalCO2emission_int(:,1)==cut_year(scenarioNum-1));
    Ia                = find(PastTotalCO2emission_int(:,1)==1765);
...
cut_year_base = cut_year;

% Clear workspace
clear Ia data_AR5 data_int data Index_NoNmissing AR52deg_data ...
      scenarioNum Index_NoNmissing Ie cut_year
...
xopt = xopt2;
%%%% baselineclear fig ans COa scale h fig gig_pos HeightFig WidthFig scale...
clear fig_pos fval exitflag figure_counter Index_NoNmissing scenarioNum...
      output tmp fval path_data path_pics exitflag

save(strcat('workspaces/JoosModel_xopt_AR5_pchip_',glue,'.mat'))
COa_base      = zeros(size(data_AR5base))*NaN;
...
% Clear workspace
clear Ia data_AR5 data_int data Index_NoNmissing AR5base_data
...
%%%% 2 degree
COa_2deg      = zeros(size(data_AR52deg))*NaN;
...
%     set(groot, 'defaultLegendInterpreter','latex');
%     
%         plot(years, data_AR5base(:,deg_Base_correspondence(scenario-1)+1), 'LineWidth', 1.5, 'Color', BrightCol(5,:))
%         plot(years, data_AR52deg(:,scenario), 'LineWidth', 1.5, 'Color', BrightCol(1,:))
%         title('Emissions CO2')
%         xlim([1990 2060])
%         h = xlabel('years');  set(h, 'Interpreter', 'latex');
