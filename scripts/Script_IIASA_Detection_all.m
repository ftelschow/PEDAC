...
for test_start = baseVec
    ...
    for method = methodVec
        ...
        detect_year     = zeros( [ T Nalt length(NbauV(1) : NbauV(2)) ] );
        thresholds_year = zeros( [ 1 Nalt length(NbauV(1) : NbauV(2)) ] );
        ...
        for scnALT = 1 : Nalt
                ...
                drift_base  = COa_bau( I_cut1 : 12 : end, scnBAU + 1 );
                drift_alter = COa_alt( I_cut1 : 12 : end, scnALT + 1 );

                % get length of the future prediction after start time
                mT = length( drift_base );
                years = detectStart( scnALT ) : ( detectStart( scnALT ) + mT - 1 );
...
%                                        method, '/ScBAU_', num2str( scnBAU ),'ScALT_', num2str( scnALT ),...
...
%                                        method, '.png' ) );
 ...
 %               print( strcat( path_pics, 'detect_', method, '/ScBAU_', num2str( scnBAU ),'ScALT_', num2str( scnALT ),...
...
%         load( strcat('workspaces/Detection_aCO2_IISA_all_2_base', base, '_',method,'.mat') )
%         
...
%                    '/Box_Detect_aCO2_IISA_all_base', base,...
