...
years = yearStart + ( 1 : size( probs, 2 ) ) - 1;
...
plot( [ years( 1 ) - 1, years( end ) + 1 ], [ q( 1 ), q( 1 ) ], 'k--' )
plot( [ years( 1 ) - 1, years( end ) + 1 ], [ q( 2 ), q( 2 ) ], 'k--' )

qyears = get_Quants( probs( 2, : ), quant );

for l = 1 : length( quant )
    qyear = qyears( l );

    if l == 2
        plot( [ years( qyear ), years( qyear ) ], [ -0.95, 2.95 ],...
              'k-', 'LineWidth', 2 )
    else
...
h = ylabel( "cummulated probability of detection" );
