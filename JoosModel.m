...
% beta = 0.8; % fertilization factor
% CO2_preind = 280; % 278 in Joos, but tweaking to match observed record
% c1 = 0.3; % sinks scaling factor
...
ff = getSourceData_fabian(ts, DATA);
...
for i = 1:length(year)-1
    ...
    dtdelpCO2a(i,2) =  ff(i,2) - c1*(Aoc*fas(i,2) + ffer(i,2)) ; % Eq. 4 (Joos '96)
    dpCO2a(i+1,2)   = dpCO2a(i,2) + dtdelpCO2a(i,2)/12; 
    CO2a(i+1,2)     = dpCO2a(i,2) + CO2a(1,2); 
    sumCheck(i,2)   = dtdelpCO2a(i,2)+Aoc*fas(i,2)+ffer(i,2)-ff(i,2);

end

% calculate final time points
fas(end,2)        = (kg/Aoc)*(dpCO2a(end,2) - dpCO2s(end,2)); 
dtdelpCO2a(end,2) =  ff(end,2) - c1*(Aoc*fas(i,2) + ffer(i,2)); 
